...
if (isset($_POST['Add_to_Cart'])) { //checking if add to cart button is clicked or not
    if (isset($_SESSION['cart'])) {  //checks if anything is in cart
        ...
        if (in_array($_POST['item_name'], $cartitems)) {  //this function checks whether the item is there in cart or not
            ...
            window.location.href='ethnic_wear.php';
            </script>";
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Ethnic Wear Section</title>
...
<body>
    <div class="page-content">
        <?php
        $stmt = $pdo->query("SELECT * FROM stock where item like '%ethnic%' or product_name like '%sherwani%'  
        or product_name like '%nehru%'");  //like checks whether the word ethnic,sherwani and nehru are in the columns item or 
